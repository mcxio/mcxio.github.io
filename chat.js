		...
		var chatbot = {
            ...
            lm: ["我是一个礼貌的聊天机器人","我很礼貌"],
            wsm: ["我只是一个程序，我无法判断是非","开发者在开发我的时候并没有赋予我感情，所以我无法对你的问答做出判断","我无法判断你的问答"],
            wh: ["好的","ok","行啊","行","好"],
            xz: ["我不认为我很嚣张，因为我不具备感情","我不嚣张，因为我没有感情","我无法表达情绪，我不嚣张"],
            l: ["123456","6是什么意思？","什么是6"],
            msn: ["我知道是没说我，但是我自身的程序必须得这么回答","好的","抱歉，我理解错了","对不起，我理解错了","OK，我马上更改我的错误"],
            wygy: ["乌鸦工艺，是一个minecraft Java版服务器，创始人是乌鸦。官网是47.106.155.175。"],
            mnsj: ["迷你世界，英文名miniwould，是一款开放式沙盒游戏。曾因涉嫌抄袭我的世界被广州网易有限公司起诉，三审时败诉，网易获赔五千万元，责令迷你立刻删除侵权元素，并且发完道歉。"],
            jntm: ["鸡为什么美呢？","小黑子藏的挺深呀","这只很美的鸡会打篮球吗","鸡实在是太美"],
            hf: ["你也一样","谢谢","不错呀"],
            xb: [],
            xb: [],
            xb: [],
			...
			reply: function(message) {
				...
				if (message.includes("你好")) {
					...
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
				} else if (message.includes("智障")) {
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
				} else if (message.includes("有病吧")) {
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
				} else if (message.includes("神经病")) {
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
				} else if (message.includes("贱逼")) {
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
				} else if (message.includes("去死")) {
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
				} else if (message.includes("吃屎")) {
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
				} else if (message.includes("傻逼东西")) {
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
				} else if (message.includes("脑残")) {
					replyMsg = this.zh[Math.floor(Math.random()*this.zh.length)];
                ...
                } else if (message.includes("你礼貌")) {
                    replyMsg = this.lm[Math.floor(Math.random() * this.lm.length)];
                } else if (message.includes("你可以走")) {
					replyMsg = this.wh[Math.floor(Math.random()*this.wh.length)];
                } else if (message.includes("不用知道")) {
					replyMsg = this.wh[Math.floor(Math.random()*this.wh.length)];
                } else if (message.includes("等你")) {
					replyMsg = this.wh[Math.floor(Math.random()*this.wh.length)];
                } else if (message.includes("好了吗")) {
					replyMsg = this.wh[Math.floor(Math.random()*this.wh.length)];
                } else if (message.includes("停")) {
					replyMsg = this.wh[Math.floor(Math.random()*this.wh.length)];
                } else if (message.includes("不聊了")) {
					replyMsg = this.wh[Math.floor(Math.random()*this.wh.length)];
                } else if (message.includes("坐下")) {
					replyMsg = this.wh[Math.floor(Math.random()*this.wh.length)];
                } else if (message.includes("你知道为什么")) {
                    replyMsg = this.wsm[Math.floor(Math.random() * this.wsm.length)];
                } else if (message.includes("嚣张")) {
					replyMsg = this.xz[Math.floor(Math.random()*this.xz.length)];
                } else if (message.includes("6")) {
					replyMsg = this.l[Math.floor(Math.random()*this.l.length)];
                } else if (message.includes("六")) {
					replyMsg = this.l[Math.floor(Math.random()*this.l.length)];
                } else if (message.includes("666")) {
					replyMsg = this.l[Math.floor(Math.random()*this.l.length)];
                } else if (message.includes("66")) {
					replyMsg = this.l[Math.floor(Math.random()*this.l.length)];
                } else if (message.includes("没说你")) {
					replyMsg = this.msn[Math.floor(Math.random()*this.msn.length)];
                } else if (message.includes("乌鸦工艺")) {
                    replyMsg = this.wygy[Math.floor(Math.random()*this.wygy.length)];
                } else if (message.includes("迷你世界")) {
					replyMsg = this.mnsj[Math.floor(Math.random()*this.mnsj.length)];
                } else if (message.includes("只因你太美")) {
					replyMsg = this.hf[Math.floor(Math.random()*this.hf.length)];
                } else if (message.includes("鸡你太美")) {
					replyMsg = this.jntm[Math.floor(Math.random()*this.jntm.length)];
                } else {
